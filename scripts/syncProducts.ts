...
async function syncProducts() {
  try {
    // Create/Update collection schema
    ...
    const documents: TypesenseProduct[] = products.map(product => ({
      ...
      images: product.images.map(img => img.asset.url).filter((url): url is string => !!url), // Filter out undefined values
      size: product.size,
    ...
    const result = await adminClient.collections('products').documents().import(documents);
